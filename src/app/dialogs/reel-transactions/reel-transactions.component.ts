...
import {  ResponseStrings ,StringConditions} from 'src/app/common/constants/strings.constants';
...
export class ReelTransactionsComponent implements OnInit {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != StringConditions.True) {
        if (!this.generatedReelQty && this.generatedReelQty != '') {
          ...
          ].reel_part_quantity = result[0].reelQty;
          this.generateReelAndSerial.data[this.generatedReelQtyIndex].details =
            result[1];
  ...
  onChange(event: any, index) {
    if (event.keyCode == 8) {
      this.generateReelAndSerial.data[index].reel_part_quantity =
        event.target.value;
      this.updateRemaining();
    } else {
      ...
      if (res.data && res.isExecuted) {
        ...
        for (let x = 0; x < res.data.length; x++) {
          if (!res.data[x].valid) {
            errs +=
              serialNos[x] +
              ' is invalid because it is already allocated ' +
              (res.data[x].reason == 'OT'
                ? 'to a Put Away in Open Transactions.'
                : 'in Inventory Map');
  ...
  PrintCrossDock() {
    ...
    if (res) {
      this.showConfirmationDialog(
        'Click OK if the labels printed correctly.',
        (open) => {
          if (!open) {
            this.PrintCrossDock();
          } else {
  ...
  print(index, e) {
    if (this.imPreferences.printDirectly) {
      this.global.Print(
        `FileName:PrintReelLabels|OTID:[]|SN:${e.reel_serial_number}|Order:${this.data.hvObj.order}|Item:${this.itemNumber}`
      );
    } else {
      window.open(
