...
export class CmPrintOptionsComponent implements OnInit {
  ...
  printNewLines() {
    if (this.print) {
      this.global.Print(
        `FileName:PrintPrevCMPackList|OrderNum:${this.orderNumber}|Where:where|OrderBy:${this.packListSort}|WSID:${this.userData.wsid}`
      );
    } else {
      window.open(
  ...
  printAllLines() {
    if (this.print) {
      this.global.Print(
        `FileName:PrintPrevCMPackList|OrderNum:${this.orderNumber}|Where:all|OrderBy:${this.packListSort}|WSID:${this.userData.wsid}`
      );
    } else {
      window.open(
